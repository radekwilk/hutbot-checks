$(document).ready(function () {
  ...
  const errorsMsg = {
    ...
    delateTab:
      "Please delate Report sheet in selected Excel file. It's causing issues with data download.",
    missingTabs:
      "Something went wrong. Please ensure you are checking valid Hutbot file.",
    noRecords: "Could not find any records for this question",
    noDeliveryFresh:
      "In last 7 days checked, there is no record of temp check for delivery of fresh products",
    noDeliveryFrozen:
      "In last 7 days checked, there is no record of temp check for delivery of frozen products",
  };
    ...
    if (inputVal) {
      ...
      if (typeOfRoutine === "task") {
        ...
        if (taskCount === 0) {
          ...
          console.log(
            `NUmber of ${task} task is ${taskCount.count}, missed are ${taskCount.missed} which is ${taskCount.percent}%`
          );
        }
      } else {
    ...
    reader.onload = function (event) {
      ...
      if (numOfSheets > 1) {
        ...
        if (sheetName) {
          ...
          console.log(myData);
          displayStoreDetails(
            storeDetailsObj.storeName,
            storeDetailsObj.storeNumber
          );
        } else {
  ...
  const countRoutines = (obj, routineName, isYesNo) => {
    ...
    for (let i = 0; i < obj.length; i++) {
      ...
      daysDifference = calcNumberOfDays(
        firstDateOnSpreadsheet,
        currentShiftDate
      );
    ...
    console.log(
      `print missed tasks in object: ${questionsObj[globalTaskName].missed}, variable: ${countMissedTask}`
    );
    console.log(
      `print incorrect tasks in object: ${questionsObj[globalTaskName].incorrect}, variable: ${countIncorrect}`
    );
  ...
  const countTask = (obj, taskName) => {
    ...
    let questionAnswer_original = ""; //This variable will hold answer given in Hutbot - to use it to highlight the missed values!!!
    let questionText = ""; //This variable will hold answer given in Hutbot in the comment box
    let routineStatus = ""; //This variable will hold what is the status of the task, i.e "ON TIME", "LATE" or "MISSED"
    // get the first date in spreadsheet and then convert it to JS date format
    firstDateOnSpreadsheet = getDate(obj);
    ...
    for (let i = 0; i < obj.length; i++) {
      ...
      if (obj[i]["Tab Name"] === taskName) {
        ...
        if (daysDifference < questionsObj.maxNumberDays) {
          ...
          questionAnswer_original = obj[i]["Question Answer"];
          questionText = obj[i]["Question Text"];
          routineStatus = obj[i]["Routine Status"];
          ...
          //  newLi.classList.add("incorrect-value");
          //}
          // if question is verified as missing, highlight it
          if (
            verifyIfMissing(
              routineStatus,
              questionAnswer_original,
              questionText
            ) === "MISSING"
          ) {
            newLi.classList.add("incorrect-value");
    ...
    // check missed %
    countTotal > 0
      ? (missedPercent = (countMissed / countTotal) * 100)
      : (missedPercent = 0);
  ...
  const checkLimit = (currentVal, limitVal, minVal, limitType) => {
    ...
    if (limitType === "hot") {
      ...
      if (currentVal > limitVal || currentVal < minVal) {
        console.log(
          `Current value: ${currentVal}, limit: ${limitVal}, Min temp: ${minVal}, Limit type:  ${limitType}`
        );
        return "incorrect";
      } else {
        return "correct";
  ...
  const displayTaskDetailsModal = (index, obj) => {
    ...
    if (index) {
      ...
      if (completionDate) {
        ...
        modalDateAnswer.innerText = dateString.toLocaleDateString(
          "en-us",
          options
        );
      } else {
  ...
  // function to check if the task is declared as missing, it must have status MISSING or there is not answer to the question and no action is taken
  const verifyIfMissing = (taskStatus, question, answer) => {
    if (
      taskStatus === "MISSED" ||
